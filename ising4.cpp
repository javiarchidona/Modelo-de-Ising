...
#define T 0.4         //Valor temperatura
#define N 30         //Dimension de la matriz
...
int main(){
    ...
    magn.open("magnetizacion_T0,4.txt");
    pos.open("posiciones.txt");
    datos3.open("T_0,4(FINAL_MAGN).txt");
...
double CalcularP(double s[N][N], int n, int m){
    ...
    if((n==0)&&(m==0)){
        E=2.0*s[n][m]*(s[n+1][m]+s[N][m]+s[n][m+1]+s[n][N]);
    }

    if((n==0)&&(m==N)){
        E=2.0*s[n][m]*(s[n+1][m]+s[N][m]+s[n][0]+s[n][m-1]);
    }

    if((n==N)&&(m==0)){
        E=2.0*s[n][m]*(s[0][m]+s[n-1][m]+s[n][m+1]+s[n][N]);
    }
    ...
    if((n==0)&&(m!=0)&&(m!=N)){
        E=2.0*s[n][m]*(s[n+1][m]+s[N][m]+s[n][m+1]+s[n][m-1]);
    }
    ...
    if((m==0)&&(n!=0)&&(n!=N)){
        E=2.0*s[n][m]*(s[n+1][m]+s[n-1][m]+s[n][m+1]+s[n][N]);
    }
